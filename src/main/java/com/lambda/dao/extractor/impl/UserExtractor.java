        ...
        return rs -> {
            ...
            while (rs.next()) {
                ...
                GrantedAuthority role = new SimpleGrantedAuthority(
                    rs.getString(UserColumn.AUTHORITY));
        ...
        return rs -> {
            ...
            while (rs.next()) {
                ...
                if (!username.equals(previousName)) {
                    ...
                    props
                        .put(UserColumn.ACCOUNT_EXPIRED, rs.getBoolean(UserColumn.ACCOUNT_EXPIRED));
                    props.put(UserColumn.ACCOUNT_LOCKED, rs.getBoolean(UserColumn.ACCOUNT_LOCKED));
                    props.put(UserColumn.CREDENTIALS_EXPIRED,
                        rs.getBoolean(UserColumn.CREDENTIALS_EXPIRED));
                ...
                if (customUserMap.get(username) != null) {
                    GrantedAuthority role = new SimpleGrantedAuthority(
                        rs.getString(UserColumn.AUTHORITY));
                    ((Set<GrantedAuthority>) customUserMap.get(username).get(UserColumn.AUTHORITY))
                        .add(role);
                }
            }
