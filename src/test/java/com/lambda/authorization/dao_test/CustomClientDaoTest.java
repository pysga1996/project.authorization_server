...
import com.lambda.dao.extractor.ClientResultSetExtractor;
...
public class CustomClientDaoTest {
    ...
    private RowMapper<CustomClient> clientMapper = new ClientResultSetExtractor();

    @Test
    public void whenFindAll_thenReturn3Records() {
        List<CustomClient> customClientList = jdbcTemplate.query("SELECT * FROM oauth_client_details", clientMapper);
        assertThat(customClientList).hasSize(3);
    }
    ...
    public void whenFindById_thenReturn1Record() {
        ...
        CustomClient fooCustomClient = namedParameterJdbcTemplate.queryForObject("SELECT * FROM oauth_client_details WHERE client_id=:id", namedParameters, clientMapper);
        assertThat(fooCustomClient).isNotNull();
        assertThat(passwordEncoder.matches("secret", fooCustomClient.getClientSecret())).isTrue();
    }
